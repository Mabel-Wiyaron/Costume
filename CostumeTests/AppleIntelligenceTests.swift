...
struct AppleIntelligenceTests {
    ...
    // Test "Experience" passed after 2.813 seconds.
    let RESULT_EXPERIENCE = ExperienceGenerable(descriptions: [
        "Led the development of an AI-powered chatbot platform by implementing new RAG features and enhancing existing functionalities.",
        "Designed and implemented backend services using Python FastAPI, improving API efficiency and scalability by 30%.",
        "Optimized frontend components using Svelte, ensuring a 20% improvement in responsiveness and usability.",
        "Collaborated with cross-functional teams to configure and validate banking products, ensuring compliance with industry standards and enhancing user experience.",
        "Led a validation campaign for a banking product, resulting in a 15% reduction in errors and a 25% increase in customer satisfaction.",
    ])
    ...
    func getLanguageModel() async -> LanguageModelProtocol {
        return AppleIntelligenceService.init(temperature: TEMPERATURE)
    }
